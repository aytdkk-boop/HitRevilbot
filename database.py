...
def create_key(telegram_id: int, duration_seconds: int) -> dict:
    ...
    key = generate_key()
    # Сохраняем в формате SQLite: 'YYYY-MM-DD HH:MM:SS'
    # Это нужно, чтобы сравнение с CURRENT_TIMESTAMP работало корректно
...
def mark_key_deleted(key: str) -> bool:
    """
    Помечает конкретный ключ как удалённый:
    expires_at = прошлое, notified = 1.
    Используется при удалении на сайте, чтобы фоновая задача
