...
def get_expired_keys_not_notified():
    """Возвращает ключи, которые только что истекли и пользователь ещё не уведомлён."""
    conn = get_conn()
    rows = conn.execute(
        """SELECT * FROM keys
...
def deactivate_user_keys(telegram_id: int) -> int:
    ...
    Ключи помечаются как истёкшие (expires_at в прошлом),
    чтобы пользователь сразу мог получить новый ключ.
    """
    conn = get_conn()

    # Считаем, сколько активных ключей у пользователя
    ...
    if count > 0:
        # Помечаем активные ключи как истёкшие
        conn.execute(
            """UPDATE keys
