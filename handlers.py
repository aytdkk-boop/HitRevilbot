...
def build_menu_text(user_key: dict | None = None) -> str:
...
async def get_key_handler(message: Message):
    ...
    if existing:
        # Считаем, сколько осталось до следующей попытки
...
async def choose_duration(callback: CallbackQuery):
    ...
    text = "\n".join([
        ...
        bold('🔑 Для получения ключа нажмите "🔑 Получить ключ"'),
        "",
        bold(f"✅ Ваш ключ : <code>{new_key['key']}</code>"),
        bold(f"🕒 Активен до : {fmt_dt(new_key['expires_at'])}"),
    ])
...
async def reset_key_handler(message: Message):
    ...
    if not existing:
        # Ключа нет — просто показываем главное меню без кнопки сброса
        text = build_menu_text(None)
        is_admin = message.from_user.id in ADMIN_IDS
        await message.answer(
            text,
    ...
    # Сбрасываем активные ключи
    count = deactivate_user_keys(message.from_user.id)

    is_admin = message.from_user.id in ADMIN_IDS

    # Показываем меню без ключа (значит, без кнопки сброса)
    text = build_menu_text(None)
    await message.answer(
        text,
        reply_markup=main_menu(is_admin=is_admin, has_key=False),
    ...
    # Дополнительно сообщаем, что ключ сброшен
    if count > 0:
        await message.answer(
