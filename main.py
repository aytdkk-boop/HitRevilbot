...
async def run_bot():
    ...
    # Запускаем фоновую проверку ключей
...
async def check_expired_keys(bot: Bot):
    """
    Каждые 30 секунд проверяем, не истекает ли у кого-то ключ
    в ближайшие 2 минуты. Если да — отправляем уведомление.
    """
    while True:
        try:
            ...
            for item in expiring:
                try:
                    key_value = item["key"]
                    text = bold(
                        f"❗Ваш ключ {key_value} истек, "
                        f"что бы использовать HITREVIL, вам необходимо получить ключ."
                    )
                    ...
                    print(f"📢 Уведомление об истечении: {item['telegram_id']} → {key_value}")
