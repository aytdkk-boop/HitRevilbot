...
def main_menu(is_admin: bool = False, has_key: bool = False) -> ReplyKeyboardMarkup:
    """
    Главное меню.
    is_admin — показывать ли кнопку админки.
    has_key  — показывать ли кнопку сброса ключа.
    """
    buttons = [[KeyboardButton(text="🔑 Получить ключ")]]

    if has_key:
        buttons.append([KeyboardButton(text="🗑 Сбросить ключ")])
